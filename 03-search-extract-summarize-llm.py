...
from role_description import CHIEF_EDITOR_SOCIAL
...
def summarize_with_claude(transcript_content):
    """Send transcript to Claude for summarization and key point extraction."""
...
def format_summary_for_social_media(summary):
    """Format the summary for social media posting."""
    formatted_summary = f"📺 Video Summary:\n\n{summary}\n\n#VideoSummary #KeyPoints"
    return formatted_summary

def save_summary_to_file(summary, video_title):
    """Save the formatted summary to a new Markdown file."""
    safe_title = clean_filename(video_title)
    filename = f"{safe_title}_summary.md"
    try:
        output_dir = 'output'  # Ensure it saves to the 'output' directory
        os.makedirs(output_dir, exist_ok=True)
        file_path = os.path.join(output_dir, filename)
...
def process_youtube_url(url, language='en'):
    """Main function to process YouTube URL, extract transcript, summarize, and save files."""
    try:
        filename = sanitize_filename(url)
        transcript = extract_transcript(url, language)
        md_content = format_transcript_as_markdown(transcript, url)
        transcript_path = save_transcript_to_file(md_content, filename)
        
        summary = summarize_with_claude(md_content)
        formatted_summary = format_summary_for_social_media(summary)
        yt = YouTube(url)
        summary_filename = save_summary_to_file(formatted_summary, yt.title)
        return transcript_path, summary_filename
    except Exception as e:
        logger.error(f"Failed to process YouTube URL: {url}. Error: {str(e)}")
        return None, None

if __name__ == "__main__":
    print("Welcome to YouTube Transcript Extractor and Summarizer!")
    ...
    while True:
        try:
            ...
            if choice.lower() in ['quit', 'bye']:
                print("Thank you for using YouTube Transcript Extractor and Summarizer. Goodbye!")
                break
            ...
            result, summary_result = process_youtube_url(youtube_url, language_code)
            if result:
                print(f"Transcript saved to {result}")
                if summary_result:
                    print(f"Summary for social media saved to {summary_result}")
            else:
